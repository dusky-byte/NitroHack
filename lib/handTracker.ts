...
// How strongly hand movement controls the UI (units per normalized unit)
const DRAG_SPEED = 5.0;
// Smoothing factor for grab-point tracking (0..1, higher = snappier)
const SMOOTHING = 0.4;

export type GestureMode = "idle" | "drag" | "zoom";
...
export interface HandTrackerCallbacks {
  ...
  /** Called when both hands pinch and spread/close: multiply by factor. */
  onZoom(factor: number): void;
  /** Called when tracker status changes (hand count, mode). */
  onStatus(status: TrackerStatus): void;
  /** Called every frame with raw landmarks for gesture classification. */
  onLandmarks?(landmarks: NormalizedLandmark[][], handedness: string[]): void;
}
...
export class HandTracker {
  ...
  private prevDragGrab: Point | null = null;
  ...
  private loop = () => {
    ...
    const handednessLabels = result.handedness.map((h) => h[0]?.categoryName ?? "?");

    // Forward raw landmarks to the gesture classifier
    this.callbacks.onLandmarks?.(result.landmarks, handednessLabels);

    this.processHands(result.landmarks, handednessLabels);
  ...
  private processHands(
    ...
    const mode: GestureMode =
      pinchedGrabs.length >= 2 ? "zoom" : pinchedGrabs.length === 1 ? "drag" : "idle";
    ...
    if (mode === "drag") {
      ...
      if (this.prevDragGrab) {
        const dx = grab.x - this.prevDragGrab.x;
        const dy = grab.y - this.prevDragGrab.y;
        if (Math.abs(dx) > 1e-4 || Math.abs(dy) > 1e-4) {
          this.callbacks.onDrag(dx * DRAG_SPEED, dy * DRAG_SPEED);
        }
      }
      this.prevDragGrab = grab;
    } else if (mode === "zoom") {
      const d = Math.hypot(
      ...
      if (this.prevZoomDist && d > 1e-4) {
        // Spread hands apart -> factor < 1 -> zoom in
  ...
  private drawOverlay(landmarks: NormalizedLandmark[][]): void {
    ...
    for (const lm of landmarks) {
      ...
      // New accent teal colors instead of amber
      ctx.strokeStyle = pinched ? "#38bdf8" : "rgba(56,189,248,0.5)";
      ...
      ctx.fillStyle = pinched ? "#38bdf8" : "rgba(56,189,248,0.7)";
      for (const [x, y] of [
        [tx, ty],
        [ix, iy],
      ]) {
